...
const withBlockCache = reducer => (state = {}, action) => {
  ...
  switch (action.type) {
    ...
    case 'REPLACE_BLOCKS_AUGMENTED_WITH_CHILDREN':
      ...
      newState.cache = { ...omit(newState.cache, action.replacedClientIds),
    ...
    case 'REMOVE_BLOCKS_AUGMENTED_WITH_CHILDREN':
      newState.cache = { ...omit(newState.cache, action.removedClientIds),
        ...fillKeysWithEmptyObject(difference(getBlocksWithParentsClientIds(action.clientIds), action.clientIds))
      };
      break;
...
const withBlockReset = reducer => (state, action) => {
  if (state && action.type === 'RESET_BLOCKS') {
    ...
    const controlledInnerBlocks = Object.keys(pickBy(state.controlledInnerBlocks));
    /**
    ...
    return { ...state,
      ...
      attributes: { ...omit(state.attributes, visibleClientIds),
        ...getFlattenedBlockAttributes(action.blocks)
      },
      order: { ...omit(state.order, visibleClientIds),
        ...omit(mapBlockOrder(action.blocks), controlledInnerBlocks)
      },
      parents: { ...omit(state.parents, visibleClientIds),
        ...mapBlockParents(action.blocks)
      },
      cache: { ...omit(state.cache, visibleClientIds),
        ...omit(mapValues(flattenBlocks(action.blocks), () => ({})), controlledInnerBlocks)
      }
    };
  }

...
function compileElementsStyles(selector, elements = {}) {
  return map(elements, (styles, element) => {
    ...
    if (!isEmpty(elementStyles)) {
      return [`.${selector} ${ELEMENTS[element]}{`, ...map(elementStyles, (value, property) => `\t${kebabCase(property)}: ${value}${element === 'link' ? '!important' : ''};`), '}'].join('\n');
    }
...
export const withBlockControls = createHigherOrderComponent(BlockEdit => props => {
  ...
  return createElement(Fragment, null, shouldDisplayControls && createElement(Fragment, null, createElement(TypographyPanel, props), createElement(BorderPanel, props), createElement(ColorEdit, props), createElement(DimensionsPanel, props)), createElement(BlockEdit, props));
}, 'withToolbarControls');
/**
...
const withElementsStyles = createHigherOrderComponent(BlockListBlock => props => {
  ...
  return createElement(Fragment, null, elements && createElement("style", {

...
import { getBlockType, getFreeformContentHandlerName, getUnregisteredTypeHandlerName, hasBlockSupport } from './registration';
...
export function getSaveElement(blockTypeOrName, attributes, innerBlocks = []) {
  ...
  if (isObject(element) && hasFilter('blocks.getSaveContent.extraProps') && !hasLightBlockWrapper) {
    /**
...
export function getCommentDelimitedContent(rawBlockName, attributes, content) {
  ...
  const blockName = startsWith(rawBlockName, 'core/') ? rawBlockName.slice(5) : rawBlockName; // @todo make the `wp:` prefix potentially configurable.
